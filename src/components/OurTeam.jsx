...
const OurTeam = () => {
  ...
  return (
    <div className="relative pt-16 bg-beige-50 min-h-screen">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4">
        ...
        <div className="relative text-center mb-16">
          ...
          <div
            className="absolute top-0 left-0 w-24 h-24 opacity-15 transform -translate-x-1/2 -translate-y-1/2"
            style={{
              backgroundImage: `url(${floralPattern2})`,
                    ...
                    <div className="mb-6">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {therapist.specialties.map((specialty, index) => (
                          <span
                            key={index}
                            className="px-3 py-1 bg-beige-100/80 backdrop-blur-sm text-brown-500 rounded-full text-sm"
                          >
                            {specialty}
                          </span>
                        ))}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {teamMembers
            ...
            .map((therapist) => (
              <div
                key={therapist.id}
                className="bg-white/90 backdrop-blur-sm rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300"
              >
                ...
                <div className="p-6">
                  <h2 className="text-xl font-medium text-brown-700 mb-2">
                    {therapist.name}
                  </h2>
                  <p className="text-brown-500 mb-4">{therapist.title}</p>
                  <p className="text-brown-500 mb-4 line-clamp-3">
                    {therapist.shortBio}
                  </p>
                  <div className="mb-4">
                    <div className="flex flex-wrap gap-2">
                      {therapist.specialties
                        ...
                        .map((specialty, index) => (
                          <span
                            key={index}
                            className="px-2 py-1 bg-beige-50/80 backdrop-blur-sm text-brown-500 rounded-full text-sm"
                          >
                            {specialty}
                          </span>
                        ))}
